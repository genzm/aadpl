...
対象siteに依存しない `Condition` site の密度は、対象frameのセルごとに
複製せず、求積の外へhoistして一度だけ加える。対象siteに依存する観測密度は
被積分関数の内側に残す。
...
LRTでは同じ節点を使うH0/H1の差によって周辺尤度の主要な求積誤差が相殺する。
Phase 13 の実測では、周辺尤度単体の `K=15/20` 差が `6.49e-4` に対し、
LRT統計量の最大差は `2.61e-9` だった。
