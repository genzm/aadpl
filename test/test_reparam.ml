...
let test_reparam_uniform_unchanged () =
  let e = sample "z" [||] D_uniform in
  let e' = Transform.Reparam.reparam e in
  (* Primitive sites still get an explicit trace-value binding. *)
  match e' with
  | Let (_, "%tr.z", Sample (_, "z", _, D_uniform), Var (_, "%tr.z")) ->
      check pass "trace binding" () ()
  | _ -> fail "expected trace-bound Sample with D_uniform"

let test_reparam_normal_structure () =
  let dist = Ast.Normal.normal ~mu:"mu" ~sigma:"sigma" in
  let e = sample "z" [||] dist in
  let e' = Transform.Reparam.reparam e in
  (* Should bind both base noise and transformed trace value. *)
  match e' with
  | Let
      ( _,
        "%u.z",
        Sample (_, "z", _, D_uniform),
        Let (_, "%tr.z", _fwd, Var (_, "%tr.z")) ) ->
...
let test_coupling_frame () =
  ...
  for i = 0 to g - 1 do
    ...
    if a <> b then fail (Printf.sprintf "coupling[%d]: sim=%g reparam=%g" i a b)
  done;
...
let test_density_consistency () =
  ...
  let z, _, _ = Ast.Simulate.simulate ~run_key:55L env e_re in
...
let test_reparam_return_value () =
  ...
  let dist = Ast.Normal.normal ~mu:"mu" ~sigma:"sigma" in
  let e =
    let_ "z" (sample "z" [||] dist)
      (prim Add [ prim Mul [ var "z"; var "z" ]; const (mk_scalar 1.0) ])
  in
  ...
  if scalar_val v_orig <> scalar_val v_reparam then
    fail
      (Printf.sprintf "return: orig=%g reparam=%g" (scalar_val v_orig)
         (scalar_val v_reparam));
...
let test_reparam_categorical_unchanged () =
  ...
  View.Buf.set w.buf 0 1.0;
  View.Buf.set w.buf 1 2.0;
  View.Buf.set w.buf 2 3.0;
  let e = sample "k" [||] (D_categorical (const w)) in
  let e' = Transform.Reparam.reparam e in
  check bool "categorical reparammed" true (Transform.Reparam.is_reparammed e');
  match e' with
  | Let (_, "%tr.k", Sample (_, "k", _, D_categorical _), Var (_, "%tr.k")) ->
      check pass "trace binding" () ()
  | _ -> fail "expected trace-bound Sample with D_categorical"
...
let test_assess_expr_scalar () =
  ...
  check (float 1e-12) "scalar assess_expr" (scalar_val ld_val)
    (scalar_val ld_sym)
...
let test_assess_expr_frame () =
  ...
  let ld_expr = Transform.Assess_expr.assess_expr ~env_shapes e slots in
  let ld_sym = Ast.Eval.eval env ld_expr in
  check (float 1e-12) "frame assess_expr" (scalar_val ld_val)
    (scalar_val ld_sym)
...
let test_assess_expr_with_score () =
  ...
  let dist = Ast.Normal.normal ~mu:"mu" ~sigma:"sigma" in
  let e =
    let_ "z" (sample "z" [||] dist) (let_ "_s" (score (var "z")) (var "z"))
  in
  let mu = mk_scalar 0.0 in
  let sigma = mk_scalar 1.0 in
  let env = [ ("mu", mu); ("sigma", sigma) ] in
...
let test_trace_compat_missing_site () =
  ...
  let raised = ref false in
  (try Transform.Reparam.check_trace_compat ~model ~guide
...
let test_trace_compat_extra_model_site () =
  let dist = Ast.Normal.normal ~mu:"mu" ~sigma:"sigma" in
  let model =
    let_ "z" (sample "z" [||] dist) (let_ "w" (sample "w" [||] dist) (var "z"))
  in
  let guide = let_ "z" (sample "z" [||] dist) (var "z") in
  let raised = ref false in
  (try Transform.Reparam.check_trace_compat ~model ~guide
   with Transform.Reparam.Trace_mismatch msg ->
     raised := true;
     check bool "mentions model-only site" true (contains msg "'w'"));
  check bool "Trace_mismatch raised" true !raised
    ...
    [
      ...
      ( "trace_compat",
        [
          ...
          test_case "extra model site" `Quick test_trace_compat_extra_model_site;
