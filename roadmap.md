...
- 観測 site は Phase 12 の `build_elbo ~observed` から、Phase 13 で
  `~slots:[(name, `Condition | `Maximize, value)]` に一般化した。
- 12-4 段 1a は閉形式事後（N=2048）、段 1b は独立共役モデルを
  `[N; G] = [512; 3]` の一つの rank に載せた言語内 VI で通過。
  15 個の事後標本に対する順位の chi-square は、段 1a が
  `z=7.875`, `log joint=22.656`、段 1b が
  `z=(9.062, 22.125, 12.562)`, `log joint=15.750`（閾値 38）。
  これは平均場近似の品質ではなく、実装が正しければ必ず通る較正検査。
- 12-5 は線形プリミティブ `Mask(value, mask)` を追加。パディング入力を
  先に安全値へ写してから密度寄与もマスクする二段構成とし、マスク外を
  `NaN`, `±∞`, `±1e300` に変えても密度と全勾配がビット不変であることを検査。
- 12-6 の `tau` guide は `LogNormal = exp_* Normal`（新しい構成子なし）。
  `[N;G]=[256;6]` を1200 stepの平均場 VIで同時学習し、ELBO改善を確認。
  SBC段2（診断、合否条件外）は chi-square が
  `mu=32.250, tau=780.375, a=18.021, log_joint=671.500`。
...
- 複製・パーティクル・節点軸は frame の先頭に積む。
- frame 付き `D_categorical` 抽出と記号的密度を 13-0 で実装し、
  ブートストラップデータも同じモデル記述から言語内で生成する。
- 初期規模は `B=200`, `K=15–20`、中間要素数 $10^7$ 以下。必要性を
  測ってから増やし、この範囲では要素ごと経路の最適化を行わない。
...
- 同じGLMM ASTで `a` を求積、`gamma/tau` を `Maximize` slotへ出す
  固定反復MLEを検査。`H0=-710.118824`, `H1=-683.100409`,
  `gamma=1.1769`, `tau=0.7844`。ロジスティックではK依存が残り、
  `K=15` と `K=20` の差は `6.49e-4` だった。
- 境界混合の原子判定は統計量の最適化許容差と、`tau` 推定値の
  境界許容差を別々に受ける。既定値0は数学上の厳密な混合分布。
- 13-4 は帰無データ `B=200` を frame 付き `D_categorical` と
  `ns_data`（予約site id 16）で言語内生成し、`[B;G;n]` の先頭軸上で
  H0/H1 の MLE を各1本の Adam ループとして実行した。原子質量 `0.575`、
  正成分85個の chi-square(1) KS距離 `0.076`、`Pr(T>1)=0.155`
  （漸近値 `0.159`）、全200標本で尤度単調性違反0。
- 原子は有限な `rho` が0へ到達することに依存させず、H0をH1の実行可能な
  境界候補として比較し、`tau <= 0.03` で分類する。`tau=0.01` の片側方向が
  境界を向いた111標本に対し、採用された境界候補は115標本だった。
  `K=15/20` で LRT 統計量の最大差は `2.61e-9` で、周辺尤度単体の
  K依存は同じ節点を使うH0/H1の差でほぼ相殺された。
- `log_marginal` 1評価（`B=1,K=20,G=64,n=20`）の profile は
